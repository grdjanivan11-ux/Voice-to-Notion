      ...
      <div className="vtn-container relative z-10 pb-16">
        <header className="flex flex-wrap items-center justify-between gap-4 py-6">
          ...
          <nav aria-label="Main navigation" className="flex flex-wrap items-center gap-3 text-xs font-semibold">
            ...
            <a href="/pricing" className="px-2 py-2 text-[var(--muted)] transition hover:text-[var(--foreground)]">Plans</a>
        ...
        <section id="plans" className="scroll-mt-10 py-14 md:py-20">
          ...
          <div className="mt-9 grid max-w-4xl gap-5 md:grid-cols-2">
            ...
            <div className="vtn-card relative overflow-hidden border-violet-500/35 p-6 sm:p-8"><div className="pointer-events-none absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-violet-400 to-cyan-400" /><div className="flex items-center justify-between"><p className="text-sm font-bold">✦ Pro</p><span className="rounded-full border border-violet-400/30 bg-violet-500/10 px-3 py-1 text-[10px] font-bold text-violet-300">SANDBOX TESTING</span></div><p className="mt-5 text-4xl font-black">€8.80 <span className="text-sm font-medium text-[var(--muted)]">/ month</span></p><p className="mt-2 text-xs text-[var(--muted)]">Or €80/year during Sandbox testing. Save about 24% annually.</p><p className="mt-4 text-sm text-[var(--muted)]">For longer recordings and more monthly captures.</p><div className="mt-7 space-y-3 text-sm"><p>✓ 500 AI captures per month</p><p>✓ Recordings up to 15 minutes</p><p>✓ Up to 5 Notion destinations</p><p>✓ Pro entitlements for planned premium features</p></div><Link href="/pricing" className="vtn-primary mt-8 inline-flex min-h-12 w-full items-center justify-center px-5 text-sm font-bold">Explore Pro →</Link><p className="mt-3 text-center text-[10px] leading-5 text-[var(--muted)]">Checkout is in Paddle Sandbox. No real charges during testing.</p></div>
          </div>
        </section>
